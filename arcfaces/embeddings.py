...
def write_visomaster_embedding_for_folder(
    folder: Path, name_override: str | None = None
) -> Path:
    folder = Path(folder)
    name = name_override or folder.name
    output_dir = folder.parent / "embeddings"
    output_dir.mkdir(parents=True, exist_ok=True)
    if name_override:
        output = output_dir / f"{name_override}{folder.name}.json"
    else:
        output = output_dir / f"{folder.name}.json"

    vectors = []
    for path in tqdm(sorted(folder.glob("*.json")), desc="Merging embeddings"):
        obj = json.loads(path.read_text())
        vectors.append(np.array(obj["face"]["embedding"], dtype=np.float32))

    mean = np.mean(np.stack(vectors, axis=0), axis=0)
    data = [{"name": name, "embedding_store": {"Inswapper128ArcFace": mean.tolist()}}]
...
def main() -> int:
    ...
    parser.add_argument("--merge-faces", "-MergeFaces", required=True)
    parser.add_argument("--name", "-Name")
    args = parser.parse_args()

    write_visomaster_embedding_for_folder(args.merge_faces, name_override=args.name)
    return 0
