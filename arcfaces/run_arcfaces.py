...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.info:
        ...
        with winreg.OpenKey(
            winreg.HKEY_CURRENT_USER, r"Software\Classes\Directory\shell\Arcfaces\command", 0, winreg.KEY_READ
        ) as key:
    ...
    if args.install:
        ...
        with winreg.CreateKeyEx(
            winreg.HKEY_CURRENT_USER, r"Software\Classes\Directory\shell\Arcfaces", 0, winreg.KEY_SET_VALUE
        ) as key:
            winreg.SetValueEx(key, "", 0, winreg.REG_SZ, "Run Arcfaces")
            winreg.SetValueEx(key, "Icon", 0, winreg.REG_SZ, str(launcher_exe))
        with winreg.CreateKeyEx(
            winreg.HKEY_CURRENT_USER,
            r"Software\Classes\Directory\shell\Arcfaces\command",
            0,
    ...
    if args.uninstall:
        ...
        winreg.DeleteKey(winreg.HKEY_CURRENT_USER, r"Software\Classes\Directory\shell\Arcfaces\command")
        winreg.DeleteKey(winreg.HKEY_CURRENT_USER, r"Software\Classes\Directory\shell\Arcfaces")
        print("Removed Arcfaces context menu entry.")
        return 0

    cmd = [str(python_exe), "-m", "arcfaces", *remainder]
